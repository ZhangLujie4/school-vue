  ...
  <div class="update-student">
    <div style="margin-top: 10px;">
      <el-input placeholder="请输入内容" v-model="input">
        <el-select v-model="select" slot="prepend" placeholder="请选择">
          <el-option value="1" label="教师工号"></el-option>
    ...
    <div>
      <el-table
        ...
        <el-table-column prop="id" label="教师工号" sortable width="150"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      selectList() {
        let _this = this;
        if (_this.input == '') {
          _this.teacherList = _this.thisList;
        } else if (_this.select == 2) {
          _this.teacherList = _this.thisList.filter(function (item) {
      ...
      submitDelete() {
        if (this.multipleSelection.length === 0) {
          ...
          this.deleteTeacher(this.deleteList);
        }
      },
      deleteTeacher() {
        let _this = this;
        postRequest('/teacher/deleteList', {
          teaIdList: _this.deleteList
        }).then(function (res) {
          let response = res.data;
          if (response.code === 0) {
            _this.updateTeacher();
      ...
      rowClick(row) {
        this.$router.push({ path: '/editTeacher', query: {id: row.id}});
      },
